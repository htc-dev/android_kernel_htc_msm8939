...
#define SAFETY_TIME_DIS_BIT		BIT(5)
#define SAFETY_TIME_EN_MASK		SMB1360_MASK(5, 4)
...
static void dump_all(void)
{
	...
	pr_info("V=%d mV,I=%d mA,T=%d C,SoC=%d%%,id=%d,id_mv=%d mV,"
			...
			host_mode,
			smb1360_is_fastchg_on(the_chip), smb1360_chg_is_taper(the_chip));
...
static int smb1360_overwrite_current_adc_gain(struct smb1360_chip *chip)
{
	int i, rc = 0;
	u8 reg_val_mapping[] = { 0xE0, 0x1D,
			0xE1, 0xE0,
			0xE2, 0x1E,
			0xE3, 0x78,
			0xE4, 0x00,
	...
	pr_debug("overwrite fg current adc gain\n");
#ifndef CONFIG_HTC_BATT_8960
	rc = smb1360_enable_fg_access(chip);
	if (rc) {
		pr_err("Couldn't request FG access rc = %d\n", rc);
	...
	for (i = 0; i < ARRAY_SIZE(reg_val_mapping); i = i + 2) {
		rc = smb1360_fg_write(chip, reg_val_mapping[i],
					reg_val_mapping[i + 1]);
		if (rc) {
			pr_err("Write fg address 0x%x failed, rc = %d\n",
						reg_val_mapping[i], rc);
			return rc;
		}
	}
	...
	if (rc) {
		pr_err("wWrite reg 0x0E failed, rc = %d\n", rc);
		return rc;
	}
...
static int smb1360_fg_config(struct smb1360_chip *chip)
{
	...
	if (chip->batt_capacity_mah != -EINVAL
		...
		if (wa_for_10mohm) {
			rc = smb1360_overwrite_current_adc_gain(chip);
			if (rc) {
				dev_err(chip->dev, "Couldn't overwrite FG current ADC gain rc=%d\n",
									rc);
				goto disable_fg;
			}
		}
...
static int smb1360_hw_init(struct smb1360_chip *chip)
{
	...
	if (flag_keep_charge_on || flag_pa_recharge) {
		...
		if (chip->safety_time == 0) {
			...
			rc = smb1360_masked_write(chip, CFG_SFY_TIMER_CTRL_REG,
				SAFETY_TIME_EN_MASK | SAFETY_TIME_MINUTES_MASK,
				reg);
			if (rc < 0) {
				dev_err(chip->dev,
	...
	if (wa_for_10mohm) {
		rc = smb1360_masked_write(chip, CHG_CURRENT_REG,
			FASTCHG_CURR_MASK | SMB1360_MASK(1, 0),
			2 << FASTCHG_CURR_SHIFT);
		if (rc) {
			dev_err(chip->dev, "Couldn't write to CHG_CURRENT_REG rc=%d\n",
									rc);
			return rc;
		}
		...
		rc = smb1360_masked_write(chip, CFG_BATT_CHG_REG,
			CHG_ITERM_MASK, 0);
		if (rc) {
			dev_err(chip->dev, "Couldn't write to CFG_BATT_CHG_REG rc=%d\n",
									rc);
			return rc;
		}

		rc = smb1360_masked_write(chip, 0x01,
			SMB1360_MASK(2, 0), 3);
		if (rc) {
			dev_err(chip->dev, "Couldn't write to 01h rc=%d\n",
									rc);
			return rc;
		}

		rc = smb1360_masked_write(chip, TEMP_COMPEN_REG,
			SMB1360_MASK(3, 0), 0x0A);
		if (rc) {
			dev_err(chip->dev, "Couldn't write to TEMP_COMPEN_REG rc=%d\n",
									rc);
			return rc;
		}
	}
	...
	rc = smb1360_overwrite_current_adc_gain(chip);
	if (rc < 0) {
		pr_err("Couldn't overwrite FG current ADC gain\n");
		return rc;
	}
#endif
				...
				const struct i2c_device_id *id)
{
	...
	if (!pid || pid == (-1)
		...
		switch (pid) {
		case 342: 
			if (pcbid > 2 || (pcbid == 2 && bid >= 6))
				wa_for_10mohm = true;
			else
				wa_for_10mohm = false;
			break;
		case 344: 
			if (pcbid > 0x80 || (pcbid == 0x80 && bid >= 5))
				wa_for_10mohm = true;
			else
				wa_for_10mohm = false;
			break;
